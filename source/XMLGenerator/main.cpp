...
void write3DModel(const string fileName, vector<Point> points  , vector<Point> normals, vector<Point> textures){
...
void generatePlane(const string fileName, float size){
    ...
    write3DModel(fileName, points,normals,textures);
    return;
}
...
void generateBoxFrontBack(float sizex, float sizey, float sizez, float divisions, vector<Point>* points, vector<Point> *normals, vector<Point> *textures){
    ...
    for (float yi = -div; yi < div; yi++) {
        for (float xi = -div; xi < div; xi++ ){
            ...
            float x1 = stepx*xi;
            float x2 = stepx*(xi+1);
            float y1t = stepyt*(div+yi);
            float y2t = stepyt*(yi+1+div);
            float x1t = stepxt*(xi+div);
            float x2t = stepxt*(xi+1+div);
            Point t1 = {x1t, y1t, 0};
            Point t2 = {x2t, y1t, 0};
            Point t3 = {x2t, y2t, 0};
            Point t4 = {x1t, y2t, 0};
...
void generateBoxUpDown(float sizex, float sizey, float sizez, float divisions, vector<Point> *points, vector<Point> *normals, vector<Point> *textures){
    ...
    for (float zi = -div; zi < div; zi++) {
        for (float xi = -div; xi < div; xi++ ){
            ...
            float z1 = stepz*zi;
            float z2 = stepz*(zi+1);
            float x1t= stepxt*(xi+div)+img;
            float x2t= stepxt*(xi+1+div)+img;
            float z1t= stepzt*(zi+div);
            float z2t= stepzt*(zi+1+div);
            Point t1 = {x1t, z1t,0};
            Point t2 = {x2t, z1t,0};
            Point t3 = {x2t, z2t,0};
            Point t4 = {x1t, z2t,0};
...
void generateBoxRightLeft(float sizex, float sizey, float sizez, float divisions, vector<Point> *points, vector<Point> *normals, vector<Point> *textures){
    ...
    for (float yi = -div; yi < div; yi++) {
        for (float zi = -div; zi < div; zi++ ){
            ...
            float z1 = stepz*zi;
            float z2 = stepz*(zi+1);
            float y1t= stepyt*(yi+div);
            float y2t= stepyt*(yi+1+div);
            float z1t= stepzt*(zi+div)+img;
            float z2t= stepzt*(zi+1+div)+img;
            Point t1 = {z1t, y1t, 0};
            Point t2 = {z2t, y1t, 0};
            Point t3 = {z2t, y2t, 0};
            Point t4 = {z1t, y2t, 0};
...
void generateBox(const string fileName, float size, int divisions){
    ...
    write3DModel(fileName, points,normals,texture);
}
//normais
void generateCone(const string fileName, float radius, float height, int slices, int stacks){
    ...
    Point orig = {0,0,0};
    Point down = {0,-1,0};
    for (float sl = 0; sl < slices; sl++) {
        float stack_relation1 = 0;
        float rad1 = radius * (1-stack_relation1);
        float angle1 = ((float) sl) * 2 * M_PI / slices;
        float angle2 = ((float) sl+1) * 2 * M_PI / slices;
        Point n3 = {sinf(angle1), cosf(atan(height/radius)), cosf(angle1)};
        Point n4 = {sinf(angle2), cosf(atan(height/radius)), cosf(angle2)};
        Point pt3 = {sinf(angle1)*rad1, 0, cosf(angle1)*rad1};
        Point pt4 = {sinf(angle2)*rad1, 0, cosf(angle2)*rad1};
        Point t3 = {static_cast<float>(0.5+0.5*sinf(angle1)*(1.0-stack_relation1)),static_cast<float>(0.5+0.5*cosf(angle1)*(1.0-stack_relation1)),0};
        Point t4 = {static_cast<float>(0.5+0.5*sinf(angle2)*(1.0-stack_relation1)),static_cast<float>(0.5+0.5*cosf(angle2)*(1.0-stack_relation1)),0};
        Point textD = {0.5,0.5,0};
        ...
        texture.push_back(textD),  texture.push_back(t4), texture.push_back(t3);
    }
    
    
    
    
    for (float st = 0; st < stacks; st++) {
        float stack_relation1 = st/stacks;
        float stack_relation2 = (st+1)/stacks;
        ...
        for (float sl = 0; sl < slices; sl++) {
            float angle1 = ((float) sl) * 2 * M_PI / slices;
            float angle2 = ((float) sl+1) * 2 * M_PI / slices;
            Point n1 = {sinf(angle2), cosf(atan(height/radius)), cosf(angle2)};
            Point n2 = {sinf(angle1), cosf(atan(height/radius)), cosf(angle1)};
            Point n3 = {sinf(angle1), cosf(atan(height/radius)), cosf(angle1)};
            Point n4 = {sinf(angle2), cosf(atan(height/radius)), cosf(angle2)};
            ...
            Point t1 = {static_cast<float>(0.5+0.5*sinf(angle2)*(1.0-stack_relation2)),static_cast<float>(0.5+0.5*cosf(angle2)*(1.0-stack_relation2)),0};
            Point t2 = {static_cast<float>(0.5+0.5*sinf(angle1)*(1.0-stack_relation2)),static_cast<float>(0.5+0.5*cosf(angle1)*(1.0-stack_relation2)),0};
            Point t3 = {static_cast<float>(0.5+0.5*sinf(angle1)*(1.0-stack_relation1)),static_cast<float>(0.5+0.5*cosf(angle1)*(1.0-stack_relation1)),0};
            Point t4 = {static_cast<float>(0.5+0.5*sinf(angle2)*(1.0-stack_relation1)),static_cast<float>(0.5+0.5*cosf(angle2)*(1.0-stack_relation1)),0};
            if (st+1 == stacks) {
                ...
                texture.push_back(t2), texture.push_back(t3), texture.push_back(t4);
            }
            else {
                points.push_back(pt2), points.push_back(pt3), points.push_back(pt4);
                points.push_back(pt4), points.push_back(pt1), points.push_back(pt2);
                normals.push_back(n2), normals.push_back(n3), normals.push_back(n4);
                normals.push_back(n4), normals.push_back(n1), normals.push_back(n2);
                texture.push_back(t2), texture.push_back(t3), texture.push_back(t4);
                texture.push_back(t4), texture.push_back(t1), texture.push_back(t2);
            }
        }
    }
    
    base.insert(base.end(), points.begin(), points.end());
    write3DModel(fileName, base,normals,texture);
}
...
void generateSphere(const string fileName, float radius, float slices, float stacks){
    ...
    write3DModel(fileName, mesh,normals,texture);
}
