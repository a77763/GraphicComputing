...
int loadModel(XMLElement* node_element, Group * group){
    ...
    while(att){
        if(!strcmp(att->Name(),"file")){ 
            ...
            if (iter == group->load_models.end() ) {
                int size = readBinaryModel(fileName, &points,&normals,&textures);
                genModel(&idv,points);
                genModelNormals(&norm, normals);
                genModelTexture(&text, textures);
                vbo =make_tuple(idv,size);
                group->load_models[fileName]= vbo;
                group->load_normals[fileName]= norm;
                group->load_textures[fileName]= text;
            }else{
                vbo = group->load_models[fileName];
            ...
            textName = att->Value();
            txt = true;
            genTexture(group,textName.c_str());
        }else{
            return 0;
        }
        att = att->Next();
    ...
    if (textName != "") {
        p = new Primitive(get<0>(vbo), get<1>(vbo), norm, get<1>(vbo), text, ambi, emis, spec, diff,textName,txt);
    }else{
        string dummy;
